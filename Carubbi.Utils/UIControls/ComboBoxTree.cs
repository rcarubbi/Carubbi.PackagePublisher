...
namespace Carubbi.Utils.UIControls
{
    ...
    public class ComboBoxTree : UserControl
    {
        ...
        {
            this.tvTreeView.Font = this.Font;
        }
        ...
        private void ToggleTreeView(object sender, EventArgs e)
        {
            if (!this.frmTreeView.Visible)
            {
                ...
                if (FitToAllNodes)
                {
                    ...
                    {
                        SizeF size = cg.MeasureString(tvTreeView.Nodes[0].Text, tvTreeView.Font);

                        frmTreeView.Height =
                        pnlTree.Height =
                        tvTreeView.Height = Convert.ToInt32(size.Width > 0? size.Width : 20) * GetTotalNodes();
                    }
                }
        ...
        {
            int total = 0;
            foreach (TreeNode item in tvTreeView.Nodes)
            {
